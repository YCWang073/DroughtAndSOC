"""Create manuscript Supplementary Figure S9.

Converted from PlotSupplementaryFig9.ipynb. Only complete figures are saved
to res/eiar_figs.
"""
...
def plot_figures9a(fig, ax):
    SSMI_SSP585 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SSMI_ssp126.nc")['ssp126_SSMI12']
    ...
    ax.set_title("Changes in the maximum Duration of SSMI(SSP1-2.6)", fontsize=16, pad=15)
...
def plot_figures9b(fig, ax):
    SPEI_SSP585 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SPEI_ssp126.nc")['ssp126_SPEI12']
...
def plot_figures9c(fig, ax):
    STI_SSP585 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_STI_ssp126.nc")['ssp126_STI12']
...
def create_figures9():
    ...
    mesh1 = plot_figures9a(fig, ax1)

    ax2 = fig.add_subplot(gs[1], projection=ccrs.Robinson(central_longitude=10))
    mesh2 = plot_figures9b(fig, ax2)

    ax3 = fig.add_subplot(gs[2], projection=ccrs.Robinson(central_longitude=10))
    mesh3 = plot_figures9c(fig, ax3)

    os.makedirs(OUTPUT_DIR, exist_ok=True)

    pdf_path = os.path.join(OUTPUT_DIR, "FigureS9.pdf")
    plt.savefig(pdf_path, dpi=600, bbox_inches='tight')
    png_path = os.path.join(OUTPUT_DIR, "FigureS9.png")
