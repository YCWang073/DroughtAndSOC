"""Create manuscript Supplementary Figure S11.

Converted from PlotSupplementaryFig11.ipynb. Only complete figures are saved
to res/eiar_figs.
"""
...
def plot_figures11a(fig, ax):
    SSMI_SSP585 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SSMI_ssp370.nc")['ssp370_SSMI12']
    ...
    ax.set_title("Changes in the maximum Duration of SSMI(SSP3-7.0)", fontsize=16, pad=15)
...
def plot_figures11b(fig, ax):
    SPEI_SSP585 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SPEI_ssp370.nc")['ssp370_SPEI12']
...
def plot_figures11c(fig, ax):
    STI_SSP585 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_STI_ssp370.nc")['ssp370_STI12']
...
def plot_figures11d(ax):
    SSMI_SSP585 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SSMI_ssp370.nc")['ssp370_SSMI12']
    SSMI_SSP126 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SSMI_ssp245.nc")['ssp245_SSMI12']
    SSMI_HISTORICAL = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SSMI_historical.nc")['historical_SSMI12']
    ...
    data_ssp245 = SSMI_SSP126.values.flatten()
    data_ssp245 = data_ssp245[~np.isnan(data_ssp245)]

    data_ssp370 = SSMI_SSP585.values.flatten()
    data_ssp370 = data_ssp370[~np.isnan(data_ssp370)]

    data = [data_hist, data_ssp245, data_ssp370]
    labels = ['Historical', 'SSP2-4.5', 'SSP3-7.0']
    colors = ['#808080', '#16a085', '#d35400']
...
def plot_figures11e(ax):
    SPEI_SSP585 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SPEI_ssp370.nc")['ssp370_SPEI12']
    SPEI_SSP126 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SPEI_ssp245.nc")['ssp245_SPEI12']
    SPEI_HISTORICAL = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_SPEI_historical.nc")['historical_SPEI12']
    ...
    data_ssp245 = SPEI_SSP126.values.flatten()
    data_ssp245 = data_ssp245[~np.isnan(data_ssp245)]

    data_ssp370 = SPEI_SSP585.values.flatten()
    data_ssp370 = data_ssp370[~np.isnan(data_ssp370)]
...
def plot_figures11f(ax):
    STI_SSP585 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_STI_ssp370.nc")['ssp370_STI12']
    STI_SSP126 = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_STI_ssp245.nc")['ssp245_STI12']
    STI_HISTORICAL = xr.open_dataset("/run/media/yue/Elements SE/008_Screening_data_CMIP6/Masks/MaxDryingDuration_STI_historical.nc")['historical_STI12']

    STI_SSP585 = STI_SSP585.where(landmask == 1)
    STI_SSP126 = STI_SSP126.where(landmask == 1)
    STI_HISTORICAL = STI_HISTORICAL.where(landmask == 1)

    data_hist = STI_HISTORICAL.values.flatten()
    data_hist = data_hist[~np.isnan(data_hist)]

    data_ssp245 = STI_SSP126.values.flatten()
    data_ssp245 = data_ssp245[~np.isnan(data_ssp245)]

    data_ssp370 = STI_SSP585.values.flatten()
    data_ssp370 = data_ssp370[~np.isnan(data_ssp370)]
...
def create_figures11():
    ...
    ax1 = fig.add_subplot(gs[0, :-1], projection=ccrs.Robinson(central_longitude=10))
    mesh1 = plot_figures11a(fig, ax1)

    ax2 = fig.add_subplot(gs[1, :-1], projection=ccrs.Robinson(central_longitude=10))
    mesh2 = plot_figures11b(fig, ax2)

    ax3 = fig.add_subplot(gs[2, :-1], projection=ccrs.Robinson(central_longitude=10))
    mesh3 = plot_figures11c(fig, ax3)
    ...
    pdf_path = os.path.join(OUTPUT_DIR, "FigureS11.pdf")
    plt.savefig(pdf_path, dpi=600, bbox_inches='tight')
    png_path = os.path.join(OUTPUT_DIR, "FigureS11.png")
