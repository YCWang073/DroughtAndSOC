...
cmap_continuous = mpl.colors.ListedColormap(['#FFF9C4', '#FFD54F', '#FF8A65',
                                             '#D84315', '#4E342E'])
cmap_continuous.set_under('white')
levels_continuous = [1, 2, 3, 4, 5]
cmap_change = mpl.colors.ListedColormap(['#1E5A96', '#508CC8', '#96BEE6',
                                         '#D2E6FA', '#FFFFFF', '#FFE6C8',
                                         '#FFBE82', '#FF9646', '#E6641E'])
...
def plot_figure5a(fig, ax):
    SPEI_HISTORICAL = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SPEI_historical.nc")['historical_SPEI12']
    ...
    cbar = add_unified_colorbar(fig, ax, sm,
                                "Historical Maximum Duration of SPEI (months)",
                                extend='neither', show_texts=False)
...
def plot_figure5b(fig, ax):
    CONTINUOUS_DRYING_HISTORICAL = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/ContinuouslyDrying_historical.nc")['drying_trend']
    CONTINUOUS_DRYING_SSP126 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/ContinuouslyDrying_ssp126.nc")['drying_trend']
    CONTINUOUS_DRYING_SSP245 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/ContinuouslyDrying_ssp245.nc")['drying_trend']
    CONTINUOUS_DRYING_SSP370 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/ContinuouslyDrying_ssp370.nc")['drying_trend']
    CONTINUOUS_DRYING_SSP585 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/ContinuouslyDrying_ssp585.nc")['drying_trend']

    CONTINUOUS_DRYING_TOTAL = (CONTINUOUS_DRYING_HISTORICAL
                               + CONTINUOUS_DRYING_SSP126
                               + CONTINUOUS_DRYING_SSP245
                               + CONTINUOUS_DRYING_SSP370
                               + CONTINUOUS_DRYING_SSP585)
    ...
    lon_min, lon_max = (CONTINUOUS_DRYING_TOTAL.lon.min().values,
                        CONTINUOUS_DRYING_TOTAL.lon.max().values)
    lat_min, lat_max = (CONTINUOUS_DRYING_TOTAL.lat.min().values,
                        CONTINUOUS_DRYING_TOTAL.lat.max().values)
    ...
    ax.set_title("Number of analysis periods with concurrent drying trends", fontsize=16, pad=15)
    ax.text(-0.1, 1.1, '(b)',
            fontweight='bold',
            transform=ax.transAxes,
            fontsize=22,
...
def plot_figure5c(fig, ax):
    SPEI_SSP585 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SPEI_ssp585.nc")['ssp585_SPEI12']
    SPEI_HISTORICAL = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SPEI_historical.nc")['historical_SPEI12']
    ...
    lon_min, lon_max = (SPEI_MAX_DUR_CHANGE.lon.min().values,
                        SPEI_MAX_DUR_CHANGE.lon.max().values)
    lat_min, lat_max = (SPEI_MAX_DUR_CHANGE.lat.min().values,
                        SPEI_MAX_DUR_CHANGE.lat.max().values)
    ...
    pie_ax.pie(
        ...
        autopct='%1.1f%%',
        startangle=90,
        wedgeprops={'edgecolor': 'w', 'alpha': 0.8, 'linewidth': 0.5,
                    'width': 0.65},
    ...
    ax.add_feature(cfeature.COASTLINE, linewidth=0.5)
    ax.add_feature(cfeature.BORDERS, linestyle=':', linewidth=0.5)
    ax.set_title("Changes in the maximum Duration of SPEI", fontsize=16,
                 pad=15)
...
def plot_figure5d(fig, ax):
    SSMI_SSP585 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SSMI_ssp585.nc")['ssp585_SSMI12']
    SSMI_HISTORICAL = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SSMI_historical.nc")['historical_SSMI12']
    ...
    lon_min, lon_max = (SSMI_MAX_DUR_CHANGE.lon.min().values,
                        SSMI_MAX_DUR_CHANGE.lon.max().values)
    lat_min, lat_max = (SSMI_MAX_DUR_CHANGE.lat.min().values,
                        SSMI_MAX_DUR_CHANGE.lat.max().values)
    ...
    ax.add_feature(cfeature.COASTLINE, linewidth=0.5)
    ax.add_feature(cfeature.BORDERS, linestyle=':', linewidth=0.5)
    ax.set_title("Changes in the maximum Duration of SSMI", fontsize=16,
                 pad=15)
...
def plot_figure5e(fig, ax):
    SPEI_SSP585 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SPEI_ssp585.nc")['ssp585_SPEI12']
    SPEI_SSP126 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SPEI_ssp126.nc")['ssp126_SPEI12']
    SPEI_HISTORICAL = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SPEI_historical.nc")['historical_SPEI12']
    ...
    ax.set_ylabel('Maximum Drought Duration (months)', fontsize=14)
    ...
    ax.set_title('Distribution of Maximum\nDrought Duration of SPEI',
                 fontsize=16, pad=15)


def plot_figure5f(fig, ax):
    SSMI_SSP585 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SSMI_ssp585.nc")['ssp585_SSMI12']
    SSMI_SSP126 = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SSMI_ssp126.nc")['ssp126_SSMI12']
    SSMI_HISTORICAL = xr.open_dataset(
        "/run/media/yue/Elements SE/008_Screening_data_CMIP6/"
        "Masks/MaxDryingDuration_SSMI_historical.nc")['historical_SSMI12']
