...
def plot_figures13a(fig, ax):
    SSMI_SOC_SSP585_SEN = xr.open_dataset(f"/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/soc/SSMI/NORMALIZE_ssp126_Max_sensitivity_soc&SSMI120.nc")['max_correlation']
    SSMI_SOC_HISTORICAL_SEN = xr.open_dataset(f"/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/soc/SSMI/NORMALIZE_historical_Max_sensitivity_soc&SSMI120.nc")['max_correlation']
    ...
    ax.set_title("Changes in the selected assiciation magnitude of $SOC$ with SSMI(5-8.5)", fontsize=14, pad=15)
    hotpoint = {
...
def plot_figures13b(fig, ax):
    ...
    ax.add_feature(cfeature.COASTLINE, linewidth=0.5)
    ax.add_feature(cfeature.BORDERS, linestyle=':', linewidth=0.5)
    ax.set_title("Changes in the selected assiciation magnitude of $SOC$ with SSMI(SSP1-2.6)", fontsize=14, pad=15)
    hotpoint = {
        'AMZ-SE': [-24.1, 1.5, -60.5, -34.5, ],
        'NAMS': [18.3, 34.9, -119.9, -78.7, ],
        'AUS': [-38.8, -10.2, 114.1, 154.5, ],
...
def plot_figures13c(fig, ax):
    SSMI_SOC_SSP585_SEN = xr.open_dataset(f"/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/soc/SPEI/NORMALIZE_ssp126_Max_sensitivity_soc&SPEI120.nc")['max_correlation']
    SSMI_SOC_HISTORICAL_SEN = xr.open_dataset(f"/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/soc/SPEI/NORMALIZE_historical_Max_sensitivity_soc&SPEI120.nc")['max_correlation']
    SSMI_SOC_SSP585_SEN = SSMI_SOC_SSP585_SEN.where(landmask == 1) - SSMI_SOC_HISTORICAL_SEN.where(landmask == 1)
    ax.set_extent([-170, 180, -60, 90], crs=ccrs.PlateCarree())
    lons = SSMI_SOC_SSP585_SEN.lon.values
    lats = SSMI_SOC_SSP585_SEN.lat.values
...
def plot_figures13d(fig, ax):
    SSMI_SOC_SSP585_SEN = xr.open_dataset(f"/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/soc/STI/NORMALIZE_ssp126_Max_sensitivity_soc&STI120.nc")['max_correlation']
    SSMI_SOC_HISTORICAL_SEN = xr.open_dataset(f"/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/soc/STI/NORMALIZE_historical_Max_sensitivity_soc&STI120.nc")['max_correlation']
    SSMI_SOC_SSP585_SEN = SSMI_SOC_SSP585_SEN.where(landmask == 1) - SSMI_SOC_HISTORICAL_SEN.where(landmask == 1)
    ax.set_extent([-170, 180, -60, 90], crs=ccrs.PlateCarree())
    lons = SSMI_SOC_SSP585_SEN.lon.values
    lats = SSMI_SOC_SSP585_SEN.lat.values
    ...
    sm.set_array(np.array([]))
    cbar = add_unified_colorbar(fig, ax, sm, "", y_offset=0.07)
    ax.text(-0.05, 1.05, '(d)', fontweight='bold', transform=ax.transAxes, fontsize=18, va='top', ha='left', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'))
    return sm

def create_figures13():
    ...
    mesh0 = plot_figures13a(fig, ax0)
    ax1 = fig.add_subplot(gs[1], projection=ccrs.Robinson(central_longitude=10))
    mesh1 = plot_figures13b(fig, ax1)
    ax2 = fig.add_subplot(gs[2], projection=ccrs.Robinson(central_longitude=10))
    mesh2 = plot_figures13c(fig, ax2)
    ax3 = fig.add_subplot(gs[3], projection=ccrs.Robinson(central_longitude=10))
    mesh3 = plot_figures13d(fig, ax3)
    plt.tight_layout()
    pdf_path = "/mnt/softwares/Drought_And_SOC/res/eiar_figs/FigureS13.pdf"
    plt.savefig(pdf_path, dpi=600, bbox_inches='tight')
    output_path = "/mnt/softwares/Drought_And_SOC/res/eiar_figs/FigureS13.png"
