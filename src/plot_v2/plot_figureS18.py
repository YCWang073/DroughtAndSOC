"""Create manuscript Supplementary Figure S18.
...
def plot_figures18a(fig, ax):
    ...
    diff = SSMI_SOC_SSP245_SEN.where(landmask == 1) - SSMI_SOC_HISTORICAL_SEN.where(landmask == 1)
    ...
    ax.set_title("Changes in the selected assiciation magnitude of $R_h$ with SPEI(SSP2-4.5)", fontsize=14, pad=15)
    return None

def plot_figures18b(fig, ax):
    SSMI_SOC_SSP370_SEN = xr.open_dataset("/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/rh/SSMI/NORMALIZE_ssp245_Max_sensitivity_rh&SSMI120.nc")['max_correlation']
...
def plot_figures18c(fig, ax):
    STI_SOC_SSP370_SEN = xr.open_dataset("/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/rh/STI/NORMALIZE_ssp245_Max_sensitivity_rh&STI120.nc")['max_correlation']
...
def create_figures18():
    ...
    mesh7 = plot_figures18a(fig, ax7)

    ax8 = fig.add_subplot(gs[1], projection=ccrs.Robinson(central_longitude=10))
    mesh8 = plot_figures18b(fig, ax8)

    ax9 = fig.add_subplot(gs[2], projection=ccrs.Robinson(central_longitude=10))
    mesh9 = plot_figures18c(fig, ax9)

    os.makedirs(OUTPUT_DIR, exist_ok=True)

    pdf_path = os.path.join(OUTPUT_DIR, "FigureS18.pdf")
    plt.savefig(pdf_path, dpi=600, bbox_inches='tight')
    png_path = os.path.join(OUTPUT_DIR, "FigureS18.png")
