...
def plot_figures14a(fig, ax):
    SSMI_SOC_SSP585_SEN = xr.open_dataset("/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/rh/SPEI/NORMALIZE_ssp585_Max_sensitivity_rh&SPEI120.nc")["max_correlation"]
    ...
    valid_all = np.concatenate([diff.values[np.isfinite(diff.values)], diff2.values[np.isfinite(diff2.values)]])
    ...
    def lonlat_to_axesxy(lon, lat):
        pt = ax.projection.transform_points(ccrs.PlateCarree(), np.asarray([lon]), np.asarray([lat]))[0, :2]
    ...
    for idx, (reg, (lat0, lat1, lon0, lon1)) in enumerate(hotpoint.items()):
        ...
        iax.text(0.02, 1.05, reg, fontsize=8, weight='bold', va='top', ha='left', transform=iax.transAxes)
    ...
    ax.set_title("Changes in the selected assiciation magnitude of $R_h$ with SPEI(SSP5-8.5)", fontsize=14, pad=15)
    return None

def plot_figures14b(fig, ax):
    SSMI_SOC_SSP585_SEN = xr.open_dataset(f"/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/rh/SSMI/NORMALIZE_ssp585_Max_sensitivity_rh&SSMI120.nc")['max_correlation']
...
def plot_figures14c(fig, ax):
    STI_SOC_SSP585_SEN = xr.open_dataset(f"/run/media/yue/Elements SE/009_PreSeasonN_data_CMIP6/rh/STI/NORMALIZE_ssp585_Max_sensitivity_rh&STI120.nc")['max_correlation']
...
def create_figures14():
    ...
    mesh0 = plot_figures14a(fig, ax0)
    ax1 = fig.add_subplot(gs[1], projection=ccrs.Robinson(central_longitude=10))
    mesh1 = plot_figures14b(fig, ax1)
    ax2 = fig.add_subplot(gs[2], projection=ccrs.Robinson(central_longitude=10))
    mesh2 = plot_figures14c(fig, ax2)
    plt.tight_layout()
    pdf_path = "/mnt/softwares/Drought_And_SOC/res/eiar_figs/FigureS14.pdf"
    plt.savefig(pdf_path, dpi=600, bbox_inches='tight')
    output_path = "/mnt/softwares/Drought_And_SOC/res/eiar_figs/FigureS14.png"
